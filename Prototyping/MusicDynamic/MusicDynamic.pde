...
color redInk, whiteInk, resetInk;
void setup() {
  //Display
  ...
  for ( int i =0; i<numberOfSongs; i++ ) {
    if ( playList[i]==null ) { //ERROR, play list is NULL
      ...
      println("TheF Play List or Sound Effects did not load properly");
      printArray(playList);
      exit();
    }
  }
  ...
  //
  //Drawing Text
  color redInk = #FF0000; //AP MiniLesson on bit, 8-bit or byte (grey scale, 256), colour
